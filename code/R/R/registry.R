rep_load_reference_manifest <- function(paths) {
  ...
  required <- c("reference_path", "output_path", "sha256", "comparison", "abs_tolerance", "rel_tolerance", "required_profile")
...
rep_script_registry <- function(profile = "full") {
  rep_assert(profile %in% c("full", "fast"), "Profile must be 'full' or 'fast', not '%s'", profile)
  scripts <- data.frame(
    order = c(1:3, 10:16, 20:23, 29:37, 40:41),
    script = c(
      ...
      "40_validate_against_reference.R",
      "41_write_output_manifest.R"
    ),
    group = c(rep("gate", 3), rep("models", 7), rep("robustness", 3), rep("full", 2),
              rep("render", 6), "full", "render", rep("gate", 2)),
    stringsAsFactors = FALSE
  )
  if (profile == "fast") scripts <- scripts[!scripts$group %in% "full", , drop = FALSE]
  scripts
}
...
rep_auxiliary_output_registry <- function() {
  ...
  data.frame(output_path = c(fast, full),
             required_profile = c(rep("fast", length(fast)), rep("full", length(full))),
             stringsAsFactors = FALSE)
}
...
rep_active_output_registry <- function(paths, profile = "full") {
  rep_assert(profile %in% c("full", "fast"), "Unknown profile: %s", profile)
  ...
  reference_active <- nzchar(references$output_path) & references$comparison != "inventory" &
    (references$required_profile == "fast" |
       (profile == "full" & references$required_profile == "full"))
  auxiliary <- rep_auxiliary_output_registry()
  auxiliary_active <- auxiliary$required_profile == "fast" |
    (profile == "full" & auxiliary$required_profile == "full")
  paths_out <- sort(unique(c(references$output_path[reference_active],
                             auxiliary$output_path[auxiliary_active])))
  rep_assert(length(paths_out) > 0L, "Output registry is empty for profile %s", profile)
  paths_out
}

rep_all_known_outputs <- function(paths) {
  retired <- "output/diagnostics/power_reference_comparison.csv"
  sort(unique(c(rep_active_output_registry(paths, "full"), rep_runtime_cache_registry(),
